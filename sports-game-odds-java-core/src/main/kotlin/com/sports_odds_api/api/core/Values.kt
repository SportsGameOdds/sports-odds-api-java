...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { SportsGameOddsInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw SportsGameOddsInvalidDataException("`$name` is not set")
            is JsonNull -> throw SportsGameOddsInvalidDataException("`$name` is null")
            else -> throw SportsGameOddsInvalidDataException("`$name` is invalid, received $this")
        }
